    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not taller_nombre:
                messages.error(request, "Celda B1 vacía. Debe contener el nombre del taller.")
                return redirect('asistencia:subir_asistencia_excel')

            try:
                taller = Taller.objects.get(nombre__iexact=taller_nombre)
            except Taller.DoesNotExist:
                messages.error(request, f"No se encontró el taller '{taller_nombre}'.")
                return redirect('asistencia:subir_asistencia_excel')

            # 2. Fecha
            fecha_cell = None
            for row in range(1, ws.max_row + 1):
                if str(ws.cell(row=row, column=1).value).strip().lower() == "fecha":
                    fecha_cell = ws.cell(row=row, column=2).value
                    break

            if not fecha_cell:
                messages.error(request, 'No se encontró la fila con "Fecha".')
                return redirect('asistencia:subir_asistencia_excel')

            # 3. Parsear fecha
            if isinstance(fecha_cell, (datetime, date)):
                fecha = fecha_cell.date() if isinstance(fecha_cell, datetime) else fecha_cell
            else:
                try:
                    fecha = datetime.strptime(str(fecha_cell).strip(), "%Y-%m-%d").date()
                except ValueError:
                    messages.error(request, "La fecha debe estar en formato YYYY-MM-DD.")
                    return redirect('asistencia:subir_asistencia_excel')
            ...
            for nombre, marca in ws.iter_rows(min_row=5, min_col=1, max_col=2, values_only=True):
                ...
                if alumno:
                    Asistencia.objects.get_or_create(
                        ...
                        taller=taller,
                        fecha=fecha,
                        defaults={'estado': 'Presente' if str(marca).strip().upper() == 'X' else 'Falta'}
                    )
                    creados += 1
                else:
                    omitidos.append(nombre.strip())

            # ✅ Crear o actualizar SesionTaller
            SesionTaller.objects.update_or_create(
                taller=taller,
                fecha=fecha,
                defaults={'estado': 'R'}
            )

            # 5. Registro de subida
            ...
            # 6. Feedback
            if creados:
                messages.success(request, f"Asistencia registrada para {creados} alumno(s).")
            else:
                messages.warning(request, "No se registró asistencia.")

            if omitidos:
                messages.warning(request, f"Alumnos no encontrados: {', '.join(omitidos)}")
