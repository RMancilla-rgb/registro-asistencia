...
class RoleBasedLoginView(LoginView):
    ...
    redirect_authenticated_user = False  # <- ¡IMPORTANTE!

    def form_valid(self, form):
        """Validación personalizada después del login."""
        user = form.get_user()

        if not hasattr(user, "rol") or user.rol is None:
            messages.error(self.request, "Este usuario no tiene un rol asignado. Contacta al administrador.")
    ...
    def get_success_url(self):
        ...
        if user.rol == 'administrador':
            ...
            messages.error(self.request, "Rol desconocido. Contacta al administrador.")
